...
export class AppAdminItems {
  ...
  printItems() {
    ...
    for (let i = 0; i < idex; i++) {
      items.push(
        <app-admin-item onClick={() => this.redirect(this.db[i].id)} class="flex-item border-center hover" article={this.db[i]} />
      )
    }
    return items;
  }
